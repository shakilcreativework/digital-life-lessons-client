...
export default function MyLessonsPage() {
    ...
    const handleToggleVisibility = async (lessonId, currentVisibility) => {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/lessons/${lessonId}/visibility`, {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ visibility: nextVisibility })
            });

            if (!response.ok) throw new Error("Server rejected visibility state change.");
                            ...
                            return (
                                <div key={lesson._id} className="p-4 flex flex-col gap-3 hover:bg-surface/10 transition-colors duration-150">
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <button
                                            onClick={() => handleToggleVisibility(lesson._id, lesson.visibility || "Private")}
                                            className={`flex items-center justify-center gap-1.5 py-2 rounded-xl border text-xs font-bold active:scale-98 ${isPublic ? "bg-success/5 border-success/10 text-success" : "bg-muted/5 border-border text-muted"
                                                }`}
                                        >
                                        ...
                                        <button
                                            onClick={() => handleToggleAccessLevel(lesson._id, lesson.accessLevel || "Free")}
                                            className={`flex items-center justify-center gap-1.5 py-2 rounded-xl border text-xs font-bold active:scale-98 relative ${isPremiumTier ? "bg-secondary/5 border-secondary/10 text-secondary" : "bg-background border-border text-foreground"
                                                }`}
                                        >
